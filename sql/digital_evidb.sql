...
-- ================================================================
-- CURSOR STORED PROCEDURE: sp_cursor_overdue_evidence
-- Flags evidence stuck in 'In Lab' with no custody movement > 7 days
-- Used by: OverdueEvidenceDAO.java -> OverdueEvidenceServlet.java
-- ================================================================

DELIMITER $$

CREATE PROCEDURE IF NOT EXISTS sp_cursor_overdue_evidence()
BEGIN
    DECLARE v_evidence_id   VARCHAR(20);
    DECLARE v_evidence_type VARCHAR(100);
    DECLARE v_case_id       VARCHAR(20);
    DECLARE v_seized_date   DATE;
    DECLARE v_last_transfer DATETIME;
    DECLARE v_days_stuck    INT;
    ...
    DECLARE cur_overdue CURSOR FOR
        SELECT
            ...
            e.case_id,
            e.seized_date,
            MAX(ct.transfer_datetime) AS last_transfer
        FROM EVIDENCE e
        LEFT JOIN CUSTODY_TRANSFER ct ON e.evidence_id = ct.evidence_id
        WHERE e.current_status = 'In Lab'
        GROUP BY e.evidence_id, e.evidence_type, e.case_id, e.seized_date
        HAVING DATEDIFF(CURDATE(),
               IFNULL(MAX(ct.transfer_datetime), e.seized_date)) > 7;

    DECLARE CONTINUE HANDLER FOR NOT FOUND SET v_done = TRUE;

    DROP TEMPORARY TABLE IF EXISTS tmp_overdue_alerts;
    CREATE TEMPORARY TABLE tmp_overdue_alerts (
        evidence_id   VARCHAR(20),
        evidence_type VARCHAR(100),
        case_id       VARCHAR(20),
        days_stuck    INT,
        last_movement VARCHAR(30),
    ...
    overdue_loop: LOOP
        FETCH cur_overdue INTO
            v_evidence_id, v_evidence_type,
            v_case_id, v_seized_date, v_last_transfer;
        IF v_done THEN LEAVE overdue_loop; END IF;

        SET v_days_stuck = DATEDIFF(CURDATE(),
                           IFNULL(v_last_transfer, v_seized_date));

        INSERT INTO tmp_overdue_alerts VALUES (
            v_evidence_id,
            v_evidence_type,
            v_case_id,
            v_days_stuck,
            IFNULL(DATE_FORMAT(v_last_transfer, '%Y-%m-%d %H:%i'), 'No transfer yet'),
            CASE
                WHEN v_days_stuck > 30 THEN 'CRITICAL'
                WHEN v_days_stuck > 14 THEN 'WARNING'
                ELSE 'NOTICE'
            END
        );
    END LOOP;
    ...
    SELECT * FROM tmp_overdue_alerts ORDER BY days_stuck DESC;
    DROP TEMPORARY TABLE tmp_overdue_alerts;
END$$
