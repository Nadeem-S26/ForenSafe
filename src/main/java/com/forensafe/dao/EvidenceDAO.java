...
public class EvidenceDAO {
    ...
    // ── Update ─────────────────────────────────────────────────────
    public boolean update(Evidence e) throws SQLException {
        String sql = "UPDATE EVIDENCE SET case_id=?, storage_id=?, evidence_type=?, " +
                     "description=?, serial_number=?, seized_date=?, seized_street=?, " +
                     "seized_area=?, seized_pincode=?, current_status=? " +
                     "WHERE evidence_id=?";
        try (Connection con = DBConnection.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1,  e.getCaseId());
            ps.setString(2,  e.getStorageId());
            ps.setString(3,  e.getEvidenceType());
            ps.setString(4,  e.getDescription());
            ps.setString(5,  e.getSerialNumber());
            ps.setDate(6,    e.getSeizedDate());
            ps.setString(7,  e.getSeizedStreet());
            ps.setString(8,  e.getSeizedArea());
            ps.setString(9,  e.getSeizedPincode());
            ps.setString(10, e.getCurrentStatus());
            ps.setString(11, e.getEvidenceId());
            return ps.executeUpdate() > 0;
        }
    }
